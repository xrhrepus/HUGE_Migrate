...
void SteeringSheep::Initialize()
{
	...
	alphaSheep->PassDataPtr(&data1, &data2, &data3, &data4);
	...
	for (int i = 0; i < num_of_sheep; i++)
	{
		...
		sheeps[i]->PassDataPtr(&data1, &data2, &data3, &data4);
	}
	...
	{
		circles[i] = Circle({ {static_cast<float>(rand() % 1200 + 0),static_cast<float>(rand() % 700 + 0) },radius_of_circle });
	}
