...
void TSkyDome::Initialize()
{
	...
	mTexture = TextureManager::Get()->LoadTexture("sunset_front.tif");
...
void TSkyDome::Render(const Camera& cam)
{
	...
	mData.wvp = H::Math::Transpose(H::Math::Matrix4::rotationX(90.0f * H::Math::Constants::DegToRad) * cam.GetViewMatrix() * cam.GetPerspectiveMatrix());
