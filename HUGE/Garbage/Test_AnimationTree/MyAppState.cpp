...
void MyAppState::Update(float deltatime)
{
	...
	mTestObject.Update(deltatime);

	mTestObject.Move(mCamera.GetDirection(), deltatime);
}
...
void MyAppState::RenderScene()
{
	...
	world = mTestObject.mTransform.GetTransformMatrix();
	//
	SimpleDraw::AddLine(world * Vector3{0.0f}, world * Vector3{ 0.0,0.0f, -50.0f }, Colors::Pink);//
	//
	comp = world * vm * pm;
	tf.world = H::Math::Transpose(world);
	tf.wvp = H::Math::Transpose(comp);
	...
	mTestObject.Render(tf, mLight,mMaterial);
