...
void MyAppState::Render()
{
	...
	//SE_STD->SetValue_DiffuseMap(mModel.materialData[0].diffuseMapId);
	...
	SE_STD->Bind();
	//SE_STD->SetValue_NormalMap(mModel.materialData[0].normalMapId);
	//SE_STD->SetValue_SpecularMap(mModel.materialData[0]. specularMapId);
	for (auto& m : mModel.meshData)
	{
		m.meshBuffer.Render();
	}
	...
	//combine
	{
		...
		mPPDraw.Begin();
		//mBloomEffect.mRT_Bloom.BindPS(0);
		mEdgeDraw.mRT.BindPS(0);
		mPPDraw.Render();
		mPPDraw.End();
