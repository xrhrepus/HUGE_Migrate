...
void MyAppState::Initialize()
{
	...
	//
	mSphere = MeshBuilder::CreateShpere(10.0f, 8, { 0.0f });
	...
	ctx1.meshBuffer = &mModel.meshData[0].meshBuffer;
	//ctx1.meshBuffer = &mModel.meshData[0].meshBuffer;
	ctx1.directionalLight = mLight;
	//ctx1.diffuse = mQuadDiffuse;
	ctx1.diffuse = mModel.materialData[0].diffuseMapId;
	ctx1.normal = mModel.materialData[0].normalMapId;
	ctx1.specular = mModel.materialData[0].specularMapId;
	ctx1.settings.specularMapWeight = 1.0f;
	ctx1.settings.normalMapWeight = 1.0f;
	...
	ctx1.settings.useShadow = false;
 	...
 	//teste toon
	toonctx.camera = mCurrentCam;
	toonctx.meshBuffer = ctx1.meshBuffer;
	toonctx.directionalLight = mLight;
	toonctx.diffuse = ctx1.diffuse;
	toonctx.normal = ctx1.normal;
	toonctx.specular = ctx1.specular;
	toonctx.material = mMaterial;
	toonctx.ramp = mRamp;
	toonctx.settings.brightness = 1.0f;
	toonctx.settings.useShadow = false;
	toonctx.settings.specularMapWeight = 1.0f;
	toonctx.settings.normalMapWeight = 1.0f;
	toonctx.settings.brightness = 1.0f;


	//
 	tobj1.SetMeshContext( ctx1);
	tobj1.SetMeshContext( toonctx);

	toonctx.diffuse = mQuadDiffuse;
	toonctx.meshBuffer = &mSphereBUffer;
