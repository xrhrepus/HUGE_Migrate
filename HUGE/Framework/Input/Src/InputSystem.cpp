...
namespace
{
	...
	H::Core::WindowMessageHandler sWindowMessageHandler;
}

BOOL CALLBACK H::Input::EnumGamePadCallback(const DIDEVICEINSTANCE* pDIDeviceInstance, VOID* pContext)
{
...
LRESULT CALLBACK H::Input::InputSystemMessageHandler(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (sInputSystem)
	{
		switch (message)
...
void InputSystem::Update()
{
	...
	//mMouseWheel = mCurrMouseWheel - mPrevMouseWheel;

	mPrevMouseX = mCurrMouseX;
	mPrevMouseY = mCurrMouseY;
	//mPrevMouseWheel = mCurrMouseWheel;
