...
{
	//camera
	mCamera.SetPosition({ 30.0f, 0.0f, 0.0f });
	mCamera.SetDirection({ -1.0f,0.0f, 0.0f });
	...
	//
	mBoneTransCtrls.resize(mNumBones);
	...
	//test arm
	mBonemesh = MeshBuilder::CreateTentacle(3, 150.0f, 10.0f, 2, { 0.0f });
	mBonemesh2 = MeshBuilder::CreateFourBoneTentacle(60.0f, 10.0f, 2, { 0.0f });
	mBoneMeshBuffer.Initialize(mBonemesh);
	mBoneMeshBuffer2.Initialize(mBonemesh2);

	mBoneVShader.Initialize(BoneVertex::Format, SKINNING_FILE_PATH);
	mBonePShader.Initialize(SKINNING_FILE_PATH, "PS");
	mBoneTfBuffer.Initialize();
...
void MyAppState::RenderScene()
{
	...
	{
		btd.boneTransform[i] = mBoneMatrix[i];

	}
 	comp = mMeshTransform * vm * pm;
	tf.world = H::Math::Transpose(mBoneMatrix[0]);
	tf.wvp = H::Math::Transpose(comp);*/

	//mTransformBuffer.Set(tf);


	//mBoneTfBuffer.Set(btd);
	mBoneVShader.Bind();
	mBonePShader.Bind();
	//mBoneTfBuffer.BindVS(4);
	//mBoneTfBuffer.BindPS(4);

	//mBoneMeshBuffer2.Render();
	
	mTentacle.Render(mTransformBuffer, tf, mBoneTfBuffer,vm, pm);
