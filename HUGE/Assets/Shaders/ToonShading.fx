...
cbuffer EdgeParamBuffer : register(b5)
{
	//threshold
    float4 edgeColor;
    float threshold;
...
Texture2D toonRampMap : register(t6);
Texture2D depthNormalTex : register(t7);
...
struct VS_OUTPUT
{
    ...
    float4 normalDepth : TEXCOORD5;
 	float2 normalDepthuv : TEXCOORD6;
...
VS_OUTPUT VS(VS_INPUT input)
{
    ...
    output.normalDepth.xyz = normalize(abs(worldNormal.xyz));
    output.normalDepth.w = output.position.z / output.position.w;
     output.normalDepthuv = output.position.xy / output.position.w;
    output.normalDepthuv = (output.normalDepthuv + 1.0f) * 0.5f;
    output.normalDepthuv.y = 1.0f - output.normalDepthuv.y;
	return output;
}
...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    //modify for toon shading  =======================================================
    float diffuseIntensity = saturate(dot(dirToLight, normal));
    float4 rampVal = toonRampMap.Sample(textureSampler, float2(diffuseIntensity, 0.5f));
    //======================================================= modify for toon shading  
    ...
    //draw edge =======================================================
    //float2 thisDNuv = input.outposition.xy / input.outposition.w;
    //thisDNuv = (thisDNuv + 1.0f) * 0.5f;
    //thisDNuv.y = 1.0f - thisDNuv.y;
     float4 thisDN = ReadDepthNormal(input.normalDepthuv);
    ...
    for (int i = 0; i < 8; ++i)
    {
        dnAround += ReadDepthNormal(input.normalDepthuv + pixAround[i] * uv_step.xy);
    }
    dnAround /= 8;
    float4 colorDark = mul((1.0f - edgeColorWeight), textureColor) + mul(edgeColorWeight, edgeColor);
    //colorDark.a = lerp(textureColor.a, edgeColor.a, edgeColorWeight);
     
    textureColor = lerp(textureColor, colorDark, step(threshold, length(thisDN - dnAround)));
    textureColor.a = edgeColor.a;
    //======================================================= draw edge
    ...
    float4 color = ((ambient + diffuse) * textureColor * brightness + specular * specularFactor) * rampVal;
